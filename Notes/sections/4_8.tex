...
\end{definition}
If we are able to put in relation such decision problems by a polytime reduction, then we can say that $L$'s difficulty is at most equal to $H$'s difficulty:
in order to express such relationship between decision problems, we use this unique notation described by a pedix $p$, meaning a polytime reduction. \vspace{7pt}

Note: in the example above, the language $L$ is difficult as $H$ and it cannot be more difficult.